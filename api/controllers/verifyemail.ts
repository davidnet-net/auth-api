...
export const resend_verification_email = async (ctx: Context) => {
	try {
		...
		const client = await getDBClient();
		if (!client) {
			log_error("resend_verification_email: DATABASE CONNECTION ERR", ctx.state.correlationID);
			ctx.response.status = 500;
			ctx.response.body = { error: "Database connection error." };
			return;
		}
		...
		if (expires < new Date()) {
			ctx.response.status = 400;
			ctx.response.body = { error: "Verification token expired. Please request a new one." };
			return;
		}
		...
		await sendEmail(
			...
			await loadEmailTemplate("email_templates/email_verification.html", {
				...
				verifyemail_url: (DA_ISPROD
					...
					: "http://localhost:5173") + "/verify/email/" + user.email_verification_token,
				expiry_date: formatDateWithUTCOffset(new Date(user.email_verification_expires)),
			}),
		);

		ctx.response.status = 200;
		ctx.response.body = { message: "Verification email resent successfully." };
	} catch (err) {
		log_error("resend_verification_email error:", err, ctx.state.correlationID);
		ctx.response.status = 500;
		ctx.response.body = { error: "Internal server error." };
	}
};
