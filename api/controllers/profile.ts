...
export const profile = async (ctx: RouterContext<"/profile/:id">) => {
	...
	if (!client) {
		log_error(
			"profile error: DATABASE CONNECTION ERR",
	...
	profile.email = filterField(
		profile.email,
		profile.email_visible,
		isFriend,
		isSelf,
	);
	user_settings.timezone = filterField(
		user_settings.timezone,
		profile.timezone_visible,
		isFriend,
		isSelf,
	);
...
export const resolveIdentifier = async (
	ctx: RouterContext<"/resolve-identifier">,
) => {
	const client = await getDBClient();
	if (!client) {
		log_error(
			"resolveIdentifier error: DATABASE CONNECTION ERR",
			ctx.state.correlationID,
		);
		ctx.response.status = 500;
		ctx.response.body = { error: "Database connection error." };
	...
	try {
		...
		log_error(
			"resolveIdentifier error: " + String(err),
			ctx.state.correlationID,
		);
		ctx.response.status = 500;
