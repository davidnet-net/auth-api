...
export const moderate_DELETE_account = async (ctx: Context) => {
    ...
    if (!client) {
        log_error("MODERATE: Delete account error: DATABASE CONNECTION ERR", ctx.state.correlationID);
    ...
    try {
        ...
        await sendEmail(
            ...
            await loadEmailTemplate("email_templates/account_moderate_deletion.html", {
                username: user[0].username,
                referenceID: String(ReferenceID),
            }),
        );
        ...
        log(ctx.state.correlationID, "MODERATE: Account deleted", "ReferenceID: " + ReferenceID)
        ...
        if (DA_ISPROD) {
            ...
            if (!kanban.ok) {
                log_error("MODERATE:  error: Couldnt connect to kanban api", kanban.statusText);
            }
        } else {
            const jwt_to = Deno.env.get("DA_JWT_SECRET"); //TODO Make an better way of internal auth.
            ...
            if (!kanban.ok) {
                log_error("MODERATE: : up error: Couldnt connect to kanban api", kanban.statusText);
            }
        }
    } catch (err) {
