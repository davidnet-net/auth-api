...
import { DBVersion, getDBClient } from "../../lib/db.ts";
...
export const deleteaccount = async (ctx: Context) => {
	...
	if (!client) {
		log_error(
			"Delete account error: DATABASE CONNECTION ERR",
...
export const requestdata = async (ctx: Context) => {
	...
	if (!client) {
		log_error(
			"Request data error: DATABASE CONNECTION ERR",
			ctx.state.correlationID,
		);
		ctx.response.status = 500;
		ctx.response.body = { error: "Database connection error." };
	...
	try {
		...
		if (lastReq.length > 0) {
			...
			if (diffMs < limitMs) {
				...
				ctx.response.body = {
					error:
						"You can only request your data once every 24 hours.",
					retry_at: retryDate,
				};
				return;
			}
		...
		(async () => {
			try {
				...
				const [
					...
					client.query(`SELECT * FROM files WHERE user_id = ?`, [
						userId,
					]),
					client.query(`SELECT * FROM audit_logs WHERE user_id = ?`, [
						userId,
					]),
					client.query(
					...
					client.query(`SELECT * FROM sessions WHERE user_id = ?`, [
						userId,
					]),
					client.query(
					...
					client.query(
						`SELECT * FROM connections WHERE friend_id = ?`,
						[userId],
					),
					client.query(
						`SELECT * FROM compliance_log WHERE user_id = ?`,
						[userId],
					),
				]);
